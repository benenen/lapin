...
func (h *Handler) CreateAccessToken(ctx context.Context, c *app.RequestContext) {
	...
	if request.Name == "" || len(request.Name) > 80 {
		writeError(c, 400, "invalid_input", "Token 名称不能为空且最多 80 字")
		return
	}
	userID := currentUser(c).ID
	tx, err := h.db.Begin(ctx)
	if err != nil {
		writeError(c, 500, "internal_error", "创建 Token 失败")
		return
	}
	...
	if count >= 10 {
		writeError(c, 409, "token_limit", "最多保留 10 个有效 Token")
		return
	}
...
func (h *Handler) ListAccessTokens(ctx context.Context, c *app.RequestContext) {
	...
	if err != nil {
		writeError(c, 500, "internal_error", "读取 Token 失败")
		return
	}
...
func (h *Handler) RevokeAccessToken(ctx context.Context, c *app.RequestContext) {
	...
	if err != nil {
		writeError(c, 400, "invalid_id", "Token ID 无效")
		return
	}
	revoked, err := database.RevokeAccessToken(ctx, h.db, id, currentUser(c).ID)
	if err != nil || !revoked {
		writeError(c, 404, "not_found", "Token 不存在")
		return
	}
