...
func decodeJSON(c *app.RequestContext, target any) bool {
	...
	if err != nil || mediaType != "application/json" {
		writeError(c, 415, errorCodeUnsupportedMedia, "请求必须使用 application/json")
		return false
	}
	body := c.Request.Body()
	if len(body) == 0 || len(body) > maxJSONBody || json.Unmarshal(body, target) != nil {
		writeError(c, 400, errorCodeInvalidJSON, "请求内容不是有效的 JSON")
		return false
	}
	return true
}
...
func writeError(c *app.RequestContext, status int, code errorCode, message string) {
