...
describe('DashboardView ownership editing', () => {
  ...
  it('discards the drafted note when the reader changes chapter', async () => {
    apiMock.getSubject.mockResolvedValueOnce(twoChapterSubject())
    const wrapper = mountDashboard()
    await flushPromises()
    ...
    expect(wrapper.get('.chapter-toolbar').classes()).toContain('is-reading')
    expect((wrapper.get('.annotation-composer .rich-editor').element as HTMLTextAreaElement).value).toBe('')
